...
function addLogoToSlide(pptSlide) {
  try {
    pptSlide.addImage({
      data: `data:image/jpeg;base64,${YESSENOV_LOGO_B64}`,
      x: 0.18, y: 5.22, w: 0.42, h: 0.42,
    });
  } catch (err) {
    logError("addLogo", err);
  }
}
...
async function buildPresentation(slides, renderedList, outputPath) {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    try {
      ...
      if (slide.type === "title") {
        buildTitleSlide(pptSlide, slide, layout);
      } else if (slide.type === "end") {
        buildEndSlide(pptSlide, slide, layout);
      } else {
        buildContentSlide(pptSlide, slide, layout);
      }
      ...
      // 4) Есенов логосы — төменгі сол бұрыш
      addLogoToSlide(pptSlide);
...
// ─── ТИТУЛ СЛАЙДЫ ─────────────────────────────────────────────────────────────
function buildTitleSlide(pptSlide, slide, layout) {
  const tz = layout.title    || { x: 0.45, y: 1.50, w: 9.10, h: 1.35 };
  const sz = layout.subtitle || { x: 0.45, y: 2.95, w: 9.10, h: 0.68 };

  pptSlide.addText(slide.title || "", {
    x: tz.x, y: tz.y, w: tz.w, h: tz.h,
    fontSize: 42, bold: true,
    color: TEXT_COLOR, fontFace: FONT_TITLE,
    align: "center", wrap: true, shadow: shadow(8),
  });

  if (slide.subtitle) {
    pptSlide.addText(slide.subtitle, {
      x: sz.x, y: sz.y, w: sz.w, h: sz.h,
      fontSize: 20, color: ACCENT_COLOR,
      fontFace: FONT_BODY, align: "center",
      italic: true, wrap: true, shadow: shadow(4),
    });
  }

  pptSlide.addText(`AI Generated  ·  ${formatDate()}`, {
    x: 0.75, y: 5.25, w: 5, h: 0.28,
    fontSize: 9, color: DIM_COLOR,
    transparency: 35, fontFace: FONT_BODY,
  });
}

// ─── МАЗМҰН СЛАЙДЫ ────────────────────────────────────────────────────────────
function buildContentSlide(pptSlide, slide, layout) {
  const tz = layout.title   || { x: 0.38, y: 0.30, w: 9.24, h: 1.00 };
  const cz = layout.content || { x: 0.38, y: 1.40, w: 9.24, h: 3.90 };

  pptSlide.addText(slide.title || "", {
    x: tz.x, y: tz.y, w: tz.w, h: tz.h,
    fontSize: 26, bold: true,
    color: TEXT_COLOR, fontFace: FONT_TITLE,
    align: "left", wrap: true, shadow: shadow(6),
  });

  if (slide.content && slide.content.length > 0) {
    const items = slide.content.map((point, idx) => ({
      text: point,
      options: {
        fontSize: 17, color: TEXT_COLOR,
        bullet: { type: "number" },
        paraSpaceAfter: 8,
        breakLine: idx < slide.content.length - 1,
      },
    }));
    pptSlide.addText(items, {
      x: cz.x, y: cz.y, w: cz.w, h: cz.h,
      fontFace: FONT_BODY, valign: "top",
      margin: [6, 6, 6, 6], shadow: shadow(4),
    });
  }
}

// ─── ҚОРЫТЫНДЫ СЛАЙДЫ ─────────────────────────────────────────────────────────
function buildEndSlide(pptSlide, slide, layout) {
  const tz = layout.title    || { x: 0.60, y: 1.80, w: 8.80, h: 1.35 };
  const sz = layout.subtitle || { x: 0.60, y: 3.25, w: 8.80, h: 0.68 };

  pptSlide.addText(slide.title || "Назарларыңызға рахмет!", {
    x: tz.x, y: tz.y, w: tz.w, h: tz.h,
  ...
  if (slide.subtitle) {
    pptSlide.addText(slide.subtitle, {
      x: sz.x, y: sz.y, w: sz.w, h: sz.h,
      fontSize: 18, color: ACCENT_COLOR,
      fontFace: FONT_BODY, align: "center",
...
// ─── ҚАУІПСІЗ СЛАЙД ───────────────────────────────────────────────────────────
function buildSafeSlide(pptSlide, slide) {
  ...
  pptSlide.addText(slide.title || "...", {
    ...
    color: TEXT_COLOR, align: "center", fontFace: FONT_TITLE,
  });
}

// ─── Shadow helper ────────────────────────────────────────────────────────────
function shadow(blur = 6) {
  return { type: "outer", color: "000000", opacity: 0.55, blur, offset: 2, angle: 45 };
}
